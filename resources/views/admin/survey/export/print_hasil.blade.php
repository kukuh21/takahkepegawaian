  ...
  <body>
    ...
    <div>
      @if($jawabansurvey != 'Kosong')
          ...
          @if($status_layanan == 'Tunggal')
          <div class="panel-body" style="margin-top: 0px;">
              <table class="tabel" width="100%">
                <thead>
                  <tr>
                    <th width="10" rowspan="2" class="top">No. Res</th>
                    <th colspan="10" class="top">{{ $jenislayanan->jenislayanan_nama }}</th>
                  </tr>
                  <tr>
                  ...
                  <tbody>
                    @php
                      $no = 1;
                    @endphp
                    @foreach ($responden->where('jenislayanan_id', $jenislayanan->jenislayanan_id) as $index => $list_responden)
                    <tr>
                        <td class="tengah" width="5">{{ $no++ }}</td>
                        @foreach ($pertanyaan as $list_pertanyaan)
                          @foreach ($jawabansurvey->where('survey_id', $list_responden->survey_id)->where('jenislayanan_id', $jenislayanan->jenislayanan_id)->where('pertanyaan_id', $list_pertanyaan->pertanyaan_id) as $list_jawaban)
                    ...
                    <tr>
                      <td>Nilai Unsur</td>
                      @foreach ($pertanyaan as $list_pertanyaan)
                      	<td class="tengah">{{ $jawabansurvey->where('jenislayanan_id', $jenislayanan->jenislayanan_id)->where('pertanyaan_id', $list_pertanyaan->pertanyaan_id)->sum('jawaban_poin') }}</td>
                      @endforeach
                      <td></td>
                    </tr>
                    <tr>
                      <td>NRR / Unsur</td>
                      @php
                        $total_responden = $responden->where('jenislayanan_id', $jenislayanan->jenislayanan_id)->count();
                        $total_pertanyaan = $pertanyaan->count();
                      @endphp
                      @foreach ($pertanyaan as $list_pertanyaan)
                        @if($total_responden != 0)
                          <td class="tengah">{{ round($jawabansurvey->where('jenislayanan_id', $jenislayanan->jenislayanan_id)->where('pertanyaan_id', $list_pertanyaan->pertanyaan_id)->sum('jawaban_poin') / $total_responden, 2) }}</td>
                        @else
                    ...
                    <tr>
                      ...
                      @foreach ($pertanyaan as $list_pertanyaan)
                        @if($total_responden != 0)
                          <td class="tengah">{{ round(($jawabansurvey->where('jenislayanan_id', $jenislayanan->jenislayanan_id)->where('pertanyaan_id', $list_pertanyaan->pertanyaan_id)->sum('jawaban_poin') / $total_responden) * (1/$total_pertanyaan), 2) }}</td>
                        @else
                          <td class="tengah">0</td>
                        @endif
                      @endforeach
                      @if($total_responden != 0)
                        <td>{{ round(($jawabansurvey->where('jenislayanan_id', $jenislayanan->jenislayanan_id)->sum('jawaban_poin') / $total_responden) * (1/$total_pertanyaan), 2) }} *</td>
                      @else
                        <td>0</td>
                      @endif
                    ...
                    <tr>
                      ...
                      @php
                      $jumlah_survey = $jawabansurvey->where('jenislayanan_id', $jenislayanan->jenislayanan_id)->sum('jawaban_poin');
                      $total_survey = $jawabansurvey->where('jenislayanan_id', $jenislayanan->jenislayanan_id)->count();
              ...
              <table class="tabel" style="margin-left: auto; margin-right: auto; margin-top: 20px; font-weight: bold; min-width="50%">
                <tr>
                  <td class="tengah" style="font-size: 20px;">IKM Layanan {{ $jenislayanan->jenislayanan_nama }}: {{ round($hasil, 2) }}</td>
                </tr>
                <tr>
                  <td class="tengah" style="font-size: 20px;">Mutu Pelayanan : {{ $ket_hasil }}</td>
                </tr>
              </table>
          </div>
          @else
              ...
              <div class="panel-body" style="margin-top: 0px;">
                  <table class="tabel" width="100%">
                    <thead>
                      <tr>
                        <th width="10" rowspan="2" class="top">No. Res</th>
                        <th colspan="10" class="top">{{ $list->jenislayanan_nama }}</th>
                      </tr>
                      <tr>
                        @foreach ($pertanyaan as $list_pertanyaan)
                          <th width="30" class="top">{{ $list_pertanyaan->pertanyaan_singkatan }}</th>
                      ...
                      <tbody>
                        @php
                          $no = 1;
                        @endphp
                        @foreach ($responden->where('jenislayanan_id', $list->jenislayanan_id) as $index => $list_responden)
                        <tr>
                            <td class="tengah" width="5">{{ $no++ }}</td>
                            @foreach ($pertanyaan as $list_pertanyaan)
                              @foreach ($jawabansurvey->where('survey_id', $list_responden->survey_id)->where('jenislayanan_id', $list->jenislayanan_id)->where('pertanyaan_id', $list_pertanyaan->pertanyaan_id) as $list_jawaban)
                              <td class="tengah">{{ $list_jawaban->jawaban_poin }}</td>
                              @endforeach
                            @endforeach
                            <td></td>
                        ...
                        <tr>
                          <td>Nilai Unsur</td>
                          @foreach ($pertanyaan as $list_pertanyaan)
                          <td class="tengah">{{ $jawabansurvey->where('jenislayanan_id', $list->jenislayanan_id)->where('pertanyaan_id', $list_pertanyaan->pertanyaan_id)->sum('jawaban_poin') }}</td>
                          @endforeach
                          <td></td>
                        </tr>
                        <tr>
                          <td>NRR / Unsur</td>
                          @php
                            $total_responden = $responden->where('jenislayanan_id', $list->jenislayanan_id)->count();
                            $total_pertanyaan = $pertanyaan->count();
                          @endphp
                          @foreach ($pertanyaan as $list_pertanyaan)
                            @if($total_responden != 0)
                              <td class="tengah">{{ round($jawabansurvey->where('jenislayanan_id', $list->jenislayanan_id)->where('pertanyaan_id', $list_pertanyaan->pertanyaan_id)->sum('jawaban_poin') / $total_responden, 2) }}</td>
                            @else
                              <td class="tengah">0</td>
                            @endif
                          @endforeach
                        ...
                        <tr>
                          ...
                          @foreach ($pertanyaan as $list_pertanyaan)
                            @if($total_responden != 0)
                              <td class="tengah">{{ round(($jawabansurvey->where('jenislayanan_id', $list->jenislayanan_id)->where('pertanyaan_id', $list_pertanyaan->pertanyaan_id)->sum('jawaban_poin') / $total_responden) * (1/$total_pertanyaan), 2) }}</td>
                            @else
                              <td class="tengah">0</td>
                            @endif
                          @endforeach
                          @if($total_responden != 0)
                            <td>{{ round(($jawabansurvey->where('jenislayanan_id', $list->jenislayanan_id)->sum('jawaban_poin') / $total_responden) * (1/$total_pertanyaan), 2) }} *</td>
                          @else
                            <td>0</td>
                          @endif
                        ...
                        <tr>
                          <td class="tengah" colspan="8">IKM Unit Pelayanan</td>
                          @php
                          $jumlah_survey = $jawabansurvey->where('jenislayanan_id', $list->jenislayanan_id)->sum('jawaban_poin');
                          $total_survey = $jawabansurvey->where('jenislayanan_id', $list->jenislayanan_id)->count();
                  ...
                  <table class="tabel" style="margin-left: auto; margin-right: auto; margin-top: 20px; font-weight: bold; min-width="50%">
                    <tr>
                      <td class="tengah" style="font-size: 20px;">IKM Layanan {{ $list->jenislayanan_nama }}: {{ round($hasil, 2) }}</td>
                    </tr>
                    <tr>
                      <td class="tengah" style="font-size: 20px;">Mutu Pelayanan : {{ $ket_hasil }}</td>
                    </tr>
